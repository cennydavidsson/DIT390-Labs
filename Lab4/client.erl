...
%  			{genserver:request({list_to_atom(_Server),list_to_atom(_Machine)}, {connect, self(), St#cl_st.nick}), St#cl_st {server = _Server, machine = _Machine }};
%  		{_, undefined} ->
%  			{{error, server_not_reached, "Server not reached"}, St};
%  		_ ->
%  			{genserver:request({list_to_atom(_Server),list_to_atom(_Machine)}, {connect, self(), St#cl_st.nick}), St#cl_st { server = undefined }}
%      end;
        Result = genserver:request({list_to_atom(_Server), list_to_atom(_Machine)}, {connect, self(), St#cl_st.nick}),
        case Result of
        ok ->
